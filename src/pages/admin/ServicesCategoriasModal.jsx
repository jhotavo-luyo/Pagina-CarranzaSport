// src/pages/admin/ServicesCategoriasModal.jsx
...
const ServicesCategoriasModal = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingCategory) {
                await updateServiceCategory(editingCategory.id_categoria_servicio, formData);
    ...
    const handleDelete = async (id_categoria_servicio) => {
        if (window.confirm('¿Estás seguro? Si la categoría tiene servicios asociados, no se podrá eliminar.')) {
            try {
                await deleteServiceCategory(id_categoria_servicio);
        ...
        <div className="fixed inset-0 bg-[#0000004b] bg-opacity-75 flex items-center justify-center z-50 p-4 animate-fadeIn" onClick={onClose}>
            ...
            >
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl sm:text-3xl font-bold text-white">Gestionar Categorías de Servicios</h2>
                ...
                <div className="flex-1 overflow-y-auto pr-2">
                    {loading ? (<div className="flex justify-center items-center h-full"><LoadingSpinner /></div>) : error ? (<p className="text-red-400 text-center">{error}</p>) : (
                        <ul className="divide-y divide-gray-700">
                            {categorias.map(cat => (
                                <li key={cat.id_categoria_servicio} className="py-3 flex justify-between items-center">
                                    ...
                                    <div className="flex space-x-3">
                                        ...
                                        <button onClick={() => handleDelete(cat.id_categoria_servicio)} className="text-red-400 hover:text-red-600" title="Eliminar"><TrashIcon className="h-5 w-5" /></button>
                                    </div>
                                </li>
                            ))}
                        </ul>
...
export default ServicesCategoriasModal;
